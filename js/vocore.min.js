$(function() {
  ...
  function getExpressName(country, remote) {
    ...
    for(var i = 0; i < s_clist.length; i++) {
      ...
      return s_clist[i][4]; // only return remote express.
    }
    // internal error: the country is not in the list.
    return "";
  }
  ...
  function updateCartList() {
    ...
    if($("#rbShipping2").prop("checked")) {
      ...
      var express = getExpressName(country, false);
  ...
  $("#lstCountry").on("click", "a", function(event) {
    var country = $.trim($(this).html());
    var express = getExpressName(country, true);

    if(express == "NONE") {
        $("#cbRemote").prop("disabled", true);
        $("#cbRemote").prop("checked", false);
    } else {
        $("#cbRemote").prop("disabled", false);
    }
  ...
  $("#mdCheckOut").on("show.bs.modal", function(event) {
    ...
    if($("#btnContinue").text() != "Continue") {
        $("#btnAlertAddr").click();
        $("#btnContinue").html("Continue<span class=\"glyphicon glyphicon-arrow-right\" aria-hidden=\"true\">");
    }
  ...
  $("#btnContinue").on("click", function() {
    ...
    var carrier = "CNRPOST";  // free shipping only for CNRPOST.
    if($("#rbShipping2").prop("checked")) {
      ...
      carrier = getExpressName(country, false);
    }

    var custom = phone + "|" + carrier;
    $("#btnContinue").before('<input type="hidden" name="custom" value="' + custom + '">');

    if($("#btnContinue").text() == "Continue") {
        warning("#lbTotal", "btnAlertAddr", "<b>VERY IMPORTANT!!</b><br/>International shipping address <b>ONLY</b> " +
                "allows 'A~Z', 'a~z', '0~9' and space. Special characters such as 'ü','ó','ß','й' in your address will " +
                "not show on the package. For better delievery, please carefully check your address in PayPal.<br/>" +
                "<br/>Click 'Go to Paypal' to continue...", -1);
        $("#btnContinue").text("Go to PayPal");
        return false;
    }
    // everything is OK, now we can go forward.
    return true;
  });
});
