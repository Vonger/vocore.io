    ...
    function trace(id) {
        ...
        $.getJSON("http://vocore.io/cgi-bin/trace?email=" + encodeURIComponent(email) + "&index=" + id, function(json) {
            ...
            if (json.status === "empty") {
                if (id === 0)
                    $("#lbTraceInfo").html("<h5>Sorry, no record yet, we might still prepare it.</h5>");
            ...
            if (json.status === "completed" || json.status === "shipped") {
                ...
                if (json.carrier === "DHL(ASIA) eCommence" || json.carrier === "Deutsche Post")
                  trace = "https://dhlecommerce.asia/Portal/Track?ref=" + json.trace;
                else if (json.carrier === "DHL Express")
                  trace = "http://www.dhl.com/en/express/tracking.html?AWB=" + json.trace;
                else
                  trace = "http://www.17track.net/en/track?nums=" + json.trace;
                ...
                cur += "Trace ID: <b><a target='_blank' style='a:hover{color:#F80}' href=\'" + trace + "\'>" + json.trace + "</a></b></small></h5>";
            } else {
