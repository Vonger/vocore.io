$(function() {
    var i, j, k, count = 0;
    var country = "Select Country", cost1, cost2, countries_name = "", countries_code = "", country_note = "";

    $.getJSON("https://vocore.io/cgi-bin/geoip", function(json) {
    ...
    $("#tbStore").on("click", "li", function() {
        $("#lbModName").html($(this).find("b").html());
        $("#lbPrice1").html($(this).find("span").html());
        $("#imgItem1").attr("src", $(this).find("div").attr("path"));
    });
    ...
    $("#lstCountry").on("click", "li", function() {
        ...
        $.getJSON("https://vocore.io/cgi-bin/geoip?code=" + countries_code[i], function(json) {
            country = json.country; cost1 = json.cost1; cost2 = json.cost2;
            update();
        });
    });
    ...
    function update() {
        ...
        if (country === "CanadaX") {
          $("#rbShipping2").prop("checked", true);
          $("#rbShipping1").attr("disabled", "disabled");
          $("#lbPostNote").html("Canada Post is unstable recently, post mode is disabled.");
        } else {
          $("#rbShipping1").removeAttr("disabled");
          $("#lbPostNote").html("");
        }

        for(i = 1, id = 1; i <= count; i++) {
            var unit_name = $("#lbItem" + i).text();
        ...
        if ($("#rbShipping1").prop("checked")) {
            ...
            carrier = "express";
            if (total >= 120) // free shipping by express when order >= 120USD
                ship = 0;
            else
        ...
        order += '<input type="hidden" name="notify_url" value="https://vocore.io/cgi-bin/notify">';
        $("#lbTotal").find("b").html(total.toFixed(2));
        $("#tbCartList").find("tbody").html(cart);
        $("#btnContinue").before(order);

        if (country === "Korea") {
          $("#lbCountryNote").html("for better delievery, please purchase from your local provider, <a href='http://www.dohasiha.com/blank'>dohasiha.com</a>.");
        } else if(country === "Canada" || country === "United States") {
          $("#lbCountryNote").html("for better delievery, please buy from your local provider, <a href='http://ameridroid.com/products?keywords=vocore'>ameridroid.com</a>.");
        } else {
          $("#lbCountryNote").html("");
        }
    }
    function trace(id) {
        ...
        email =  $("#edTrace").val();
        $.getJSON("https://vocore.io/cgi-bin/trace?email=" + encodeURIComponent(email) + "&index=" + id, function(json) {
            if (id === 0)
                $("#lbTraceInfo").html("");
            ...
            if (json.status === "completed" || json.status === "shipped") {
                var suffix = json.trace.substr(json.trace.length - 2);
                var header = json.trace.substr(0, 2);
                if (suffix === "CN") {
                  if (header === "LT") json.carrier = "China EMS";
                  else json.carrier = "China Post";
                }
                if (suffix === "HK")
                    json.carrier = "HongKong Post";
                if (suffix === "40" && json.trace.length === 16) {
                  json.carrier = "Deutsche Post";
                  info = "http://pfc.walltechsystem.cn/";
                } else if ((json.carrier === "DHL(ASIA) eCommence" || json.carrier === "Deutsche Post") && suffix === "DE") {
                  info = "https://dhlecommerce.asia/track/Track?ref=" + json.trace;
                } else if (json.carrier === "DHL Express") {
                  info = "http://www.dhl.com/en/express/tracking.html?AWB=" + json.trace;
                } else if (header === "RR" && (suffix === "VU" || suffix === "TO")) {
                  json.carrier = "USPS";
                ...
                cur += json.country + ".</h5><h5><small>Contains: " + json.cart.substr(0, json.cart.length - 2) + ". ";
